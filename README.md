...
### Project Description
NPC ANO is a satirical cyber-diagnostic system that determines whether you possess authentic free will or are merely running on pre-compiled background NPC scripts. It combines a humorous multi-factor psychological scenario quiz, real-time reflex challenges, and optional optical motion analysis.
...
- A 6-stage psychological decision-tree scenario assessment.
- A timed reflex latency challenge measuring decision paralysis.
- An optional 5-second optical webcam movement scanner.
- Instant classification into 6 archetype tiers (from 0% Pure Protagonist to 99% Hardcoded Ambient Prop).
...
# Screenshots
![Landing Screen](./screenshots/hero_landing.png)
*Landing Screen: Cyberpunk diagnostic terminal with quick test access and system status.*

![Psychological Assessment](./screenshots/quiz_question.png)
*Psychological Assessment: Humorous scenario evaluation and reflex challenge.*

![Diagnostic Result](./screenshots/diagnostic_result.png)
*Diagnostic Result: NPC classification score breakdown, dialogue loops, and behavioral traits.*

![Specimen ID Badge](./screenshots/specimen_id_card.png)
*Specimen ID Card: Digital holographic badge exportable for social sharing.*

![Biometric Scanner](./screenshots/biometric_scanner.png)
...
```mermaid
graph TD
    A[User Enters NPC ANO] --> B[Launch Diagnostic Flow]
    B --> C[Scenario Assessment: 6 Decision Dilemmas]
    C --> D[Reflex Latency Tracker]
    D --> E{Optional Biometric Scan?}
    E -- Yes --> F[5-sec Optical Motion Analysis]
    E -- No --> G[Direct Result Calculation]
    F --> G
    G --> H[Fictional NPC Score Algorithm]
    H --> I[Result Modal & Archetype Assignment]
    I --> J[Generate Specimen ID Badge]
    I --> K[Save to Local Hall of Fame]
```
