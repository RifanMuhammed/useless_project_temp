...
export const ResultModal: React.FC<ResultModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-3 sm:p-6 overflow-hidden">
      <div className="relative w-full max-w-3xl max-h-[92vh] flex flex-col bg-cyber-panel border-2 border-cyber-green rounded-xl box-glow-green hud-corner-box shadow-2xl overflow-hidden">
        {/* Fixed Top Header */}
        <div className="flex items-center justify-between border-b border-cyber-border p-4 sm:px-6 shrink-0 bg-black/40">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-cyber-green animate-ping" />
            <span className="font-display font-bold text-xs sm:text-sm tracking-widest text-cyber-green">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 custom-scrollbar">
          {showProfileCard ? (
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-center bg-cyber-surface/60 p-5 rounded-xl border border-cyber-borderSubtle">
                ...
                <div className="md:col-span-5 flex flex-col items-center justify-center">
                  <div className="relative w-36 h-36 sm:w-44 sm:h-44 flex items-center justify-center">
                    ...
                    <div className="absolute flex flex-col items-center justify-center text-center">
                      ...
                      <span className="font-display text-3xl sm:text-5xl font-black text-white glow-green">
                        {animatedScore}%
                      </span>
              ...
              <div className="bg-cyber-purple/10 border border-cyber-purple/30 rounded-xl p-4 font-mono-tech text-xs text-purple-200 flex items-start gap-3">
                <Sparkles className="w-5 h-5 text-cyber-purple shrink-0 mt-0.5 animate-pulse" />
                <div>
                  <div className="font-display font-bold text-sm text-white mb-1 tracking-wider uppercase">
                    Diagnosed Behavior Profile:
                  </div>
                  <div className="leading-relaxed font-semibold text-slate-200">
                    {result.behaviorSummary}
                  </div>
                </div>
              </div>

              {/* Multi-Vector Sub-Scores Breakdown */}
              {result.subScores && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-display text-xs font-bold text-white tracking-wider">
                      DETAILED DIAGNOSTIC SUB-SCORES
                    </h4>
                    {result.diagnosticConfidence && (
                      <span className="text-[11px] font-mono-tech font-bold text-cyber-green bg-cyber-green/10 px-2.5 py-0.5 rounded border border-cyber-green/30">
                        CALIBRATED CONFIDENCE: {result.diagnosticConfidence}%
                      </span>
                    )}
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 font-mono-tech text-xs">
                    <div className="bg-cyber-surface/50 border border-cyber-borderSubtle p-3 rounded-lg">
                      <span className="text-[10px] text-slate-400 block font-bold">KINETIC VARIANCE</span>
                      <span className="text-[9px] text-slate-500 block mb-1">Body Movement</span>
                      <span className="font-bold text-cyber-cyan text-base">{result.subScores.kineticVariance}%</span>
                      <div className="w-full h-1 bg-black/50 rounded-full mt-1.5 overflow-hidden">
                        <div className="h-full bg-cyber-cyan" style={{ width: `${result.subScores.kineticVariance}%` }} />
                      </div>
                    </div>
                    <div className="bg-cyber-surface/50 border border-cyber-borderSubtle p-3 rounded-lg">
                      <span className="text-[10px] text-slate-400 block font-bold">CONVERSATIONAL ENTROPY</span>
                      <span className="text-[9px] text-slate-500 block mb-1">Dialogue Wildness</span>
                      <span className="font-bold text-cyber-purple text-base">{result.subScores.conversationalEntropy}%</span>
                      <div className="w-full h-1 bg-black/50 rounded-full mt-1.5 overflow-hidden">
                        <div className="h-full bg-cyber-purple" style={{ width: `${result.subScores.conversationalEntropy}%` }} />
                      </div>
                    </div>
                    <div className="bg-cyber-surface/50 border border-cyber-borderSubtle p-3 rounded-lg">
                      <span className="text-[10px] text-slate-400 block font-bold">ENV. COMPLIANCE</span>
                      <span className="text-[9px] text-slate-500 block mb-1">Routine Habit</span>
                      <span className="font-bold text-cyber-green text-base">{result.subScores.environmentalCompliance}%</span>
                      <div className="w-full h-1 bg-black/50 rounded-full mt-1.5 overflow-hidden">
                        <div className="h-full bg-cyber-green" style={{ width: `${result.subScores.environmentalCompliance}%` }} />
                      </div>
                    </div>
                    <div className="bg-cyber-surface/50 border border-cyber-borderSubtle p-3 rounded-lg">
                      <span className="text-[10px] text-slate-400 block font-bold">ROGUE PROTAGONIST</span>
                      <span className="text-[9px] text-slate-500 block mb-1">Hero Energy</span>
                      <span className="font-bold text-cyber-amber text-base">{result.subScores.rogueProtagonistIndex}%</span>
                      <div className="w-full h-1 bg-black/50 rounded-full mt-1.5 overflow-hidden">
                        <div className="h-full bg-cyber-amber" style={{ width: `${result.subScores.rogueProtagonistIndex}%` }} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Metrics Breakdown & Latency Grid */}
              <div>
                <h4 className="font-display text-xs font-bold text-white tracking-wider mb-2.5">
                  SPEED & BIOMETRIC METRICS
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 font-mono-tech text-xs">
                  <div className="bg-cyber-surface/50 border border-cyber-borderSubtle p-3 rounded-lg">
                    <span className="text-[10px] text-slate-400 block font-bold">REACTION LATENCY</span>
                    <span className="text-[9px] text-slate-500 block mb-1">Answer Speed (ms)</span>
                    <span className="font-bold text-white text-base">
                      {result.reactionLatencyMs ? `${result.reactionLatencyMs}ms` : '412.5ms'}
                    </span>
                  </div>
                  <div className="bg-cyber-surface/50 border border-cyber-borderSubtle p-3 rounded-lg">
                    <span className="text-[10px] text-slate-400 block font-bold">DECISION PARALYSIS</span>
                    <span className="text-[9px] text-slate-500 block mb-1">Hesitation Level</span>
                    <span className="font-bold text-cyber-crimson text-base">
                      {result.decisionParalysisIndex !== undefined ? `${result.decisionParalysisIndex}%` : '34%'}
                    </span>
                  </div>
                  <div className="bg-cyber-surface/50 border border-cyber-borderSubtle p-3 rounded-lg">
                    <span className="text-[10px] text-slate-400 block font-bold font-mono">PATH REPETITION</span>
                    <span className="text-[9px] text-slate-500 block mb-1">Patrol Loop %</span>
                    <span className="font-bold text-cyber-green text-base">{result.metrics.pathRepetition}%</span>
                  </div>
                  <div className="bg-cyber-surface/50 border border-cyber-borderSubtle p-3 rounded-lg">
                    <span className="text-[10px] text-slate-400 block font-bold">IDLE POSTURE LOCK</span>
                    <span className="text-[9px] text-slate-500 block mb-1">Stillness Factor</span>
                    <span className="font-bold text-cyber-purple text-base">{result.metrics.idleBehavior}%</span>
                  </div>
                </div>
              </div>

              {/* Detailed Contextual Behavioral Observations */}
              <div className="bg-cyber-surface/40 border border-cyber-borderSubtle rounded-lg p-4 font-mono-tech">
                <div className="flex items-center gap-2 text-cyber-green mb-2.5">
                  ...
                  <span className="font-display font-bold text-xs text-white tracking-wider">
                    5-POINT DIAGNOSTIC SYSTEM OBSERVATIONS
                  </span>
                </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 sm:px-6 border-t border-cyber-borderSubtle shrink-0 bg-black/60">
          <button
            onClick={onScanAgain}
